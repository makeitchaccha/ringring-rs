...
struct CalligraphyContext<'a> {
    pixmap: &'a mut Pixmap,
    font_system: &'a mut FontSystem,
    swash_cache: &'a mut SwashCache,
}

impl<'a> CalligraphyContext<'a> {
    fn draw_text(&mut self, text: &str, font_size: f32, x: f32, y: f32, color: Color) {
        ...
        let size = IntSize::from_wh(self.pixmap.width(), self.pixmap.height()).unwrap();
